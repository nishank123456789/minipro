...
class ManualInpainter:
    ...
    def inpaint(self, image, mask):
        ...
        while np.any(work_mask > 0):
            ...
            if best_patch is not None:
                ...
                y1, x1 = best_patch
                try:
                    result[y - ph//2:y + ph//2 + 1, x - ph//2:x + ph//2 + 1] = result[y1 - ph//2:y1 + ph//2 + 1, x1 - ph//2:x1 + ph//2 + 1]
                    work_mask[y - ph//2:y + ph//2 + 1, x - ph//2:x + ph//2 + 1] = 0
                except:
                    break
            else:
                break
    ...
    def find_best_patch(self, image, mask, point):
        ...
        for i in range(0, h - ph + 1, self.stride):
            for j in range(0, w - ph + 1, self.stride):
                if np.any(mask[i:i + ph, j:j + ph]):
                    continue

                candidate = image[i:i + ph, j:j + ph]

                if candidate.shape != patch.shape:
                    continue

                score = np.sum((candidate - patch)**2)

                if score < best_score:
                    best_score = score
                    best_patch = (i + ph//2, j + ph//2)
...
def process_folder(folder_type, data_dir, masks_dir, output_dir):
    print(f"🔧 Inpainting {folder_type} images...")
    ...
    inpainter = ManualInpainter(patch_size=9, stride=10)

    # Traverse each case subfolder
